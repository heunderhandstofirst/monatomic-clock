...
class DublinSign {
    constructor() {
      ...
      this.Wwh = [10, 15]; // RELATIVE WIDTH and HEIGHT OF SIGN
...
//   }
    ...
    let greenShade = map(sin(signTime * 0.005), -1, 1, 100, 255); // Vary green intensity over time
    stroke(0, greenShade, 0); // Set stroke color to varying shades of green
    strokeWeight(this.unit / 10); // Set stroke weight for neon effect

    // Draw a simple harp shape
    beginShape();
    vertex(-this.unit * 5, -this.unit * 10);
    vertex(-this.unit * 3, this.unit * 10);
    vertex(this.unit * 3, this.unit * 10);
    vertex(this.unit * 5, -this.unit * 10);
    endShape(CLOSE);

    // Draw strings of the harp
    for (let i = -4; i <= 4; i++) {
        line(i * this.unit, -this.unit * 10, i * this.unit, this.unit * 10);
    }
