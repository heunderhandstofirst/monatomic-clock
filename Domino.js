...
class DominoSign {
    constructor() {
      ...
      this.DominoNeon = loadImage("images/Domino/domino-neon.png");
      this.DominoBackGrnd = loadImage("images/Domino/domino-background.png"); 
      this.DominoBackGrndRed = loadImage("images/Domino/domino-background-red.png"); 
      this.DominoBackGrndOrange = loadImage("images/Domino/domino-background-orange.png"); 
      this.unit=this.WW/54   
      
      // Generate random positions for X's
      this.xPositions = [];
      var startX = -this.unit * 22.6;
      var endX = this.unit * 9.1;
      var spacing = this.unit * 2;

      for (var i = startX; i <= endX - this.unit * 2; i += spacing * 2) {
        for (var j = -this.unit * 15; j < windowHeight - this.unit * 2; j += this.unit * 4) {
    ...
    drawDominoGirders(){
      // Draw shadow girders
        ...
        var startX = -this.unit * 22.6;
        var endX = this.unit * 9.1 ;
        var spacing = this.unit * 2; // Set equal spacing

        for (var i = startX; i <= endX; i += spacing)  line(i, -this.unit * 15, i, windowHeight); // Vertical lines extending past the bottom  
        line(endX, -this.unit * 15, endX, windowHeight); // Ensure the far right vertical line at 9.1 * this.unit
        for (var j = -this.unit * 15; j <= windowHeight; j += this.unit * 2) line(startX, j, endX, j); // Horizontal lines extending from -22.6 * this.unit to 9.1 * this.unit
      
        // Draw diagonal lines to form X's in some rectangles randomly
        for (var pos of this.xPositions) {
